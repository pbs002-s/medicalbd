      ...
      <div className="bg-white rounded-3xl max-w-3xl w-full shadow-2xl border border-slate-100 overflow-hidden relative my-4 flex flex-col max-h-[92vh] animate-slide-up">
        ...
        <div className="p-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="font-bold text-slate-800 font-bangla text-sm">
        ...
        <div id="printable-prescription" className="p-6 sm:p-10 overflow-y-auto bg-white space-y-6 font-bangla text-slate-800">
          ...
          <div className="border-b-2 border-blue-600 pb-4 flex flex-col sm:flex-row justify-between items-start sm:items-end gap-3">
            <div>
              ...
              <p className="text-xs text-slate-600 mt-0.5">{prescription.doctorHospital}</p>
            </div>

            <div className="text-left sm:text-right text-xs text-slate-500">
              <span className="font-bold text-slate-800 block">স্বাস্থ্যসেতু ডিজিটাল প্রেসক্রিপশন</span>
              <span>তারিখ: <strong className="text-slate-800">{prescription.dateBn}</strong></span>
              <span className="block text-[11px] font-mono text-slate-400">{prescription.prescriptionNumber}</span>
            </div>
          </div>
          ...
          <div className="bg-slate-50 p-3.5 rounded-2xl border border-slate-200 grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
            <div>
              ...
              <strong className="text-slate-900">{prescription.patientNameBn}</strong>
            </div>
            <div>
              <span className="text-slate-400 block text-[10px]">বয়স / লিঙ্গ:</span>
              <strong className="text-slate-900">{toBn(prescription.patientAge)} বছর / পুরুষ</strong>
            </div>
            <div>
            ...
            <div>
              <span className="text-slate-400 block text-[10px]">পরিচিত এলার্জি:</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 min-h-[380px]">
            ...
            <div className="md:col-span-4 border-r border-slate-200/80 pr-4 space-y-5 text-xs">
              {/* Chief Complaints */}
              <div>
                <h4 className="font-bold text-slate-900 uppercase text-[11px] tracking-wider text-blue-900 border-b border-slate-200 pb-1 mb-2">
                  C/C (উপসর্গ):
                </h4>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-1.5 text-[11px]">
                  <div className="bg-slate-50 p-1.5 rounded-lg border border-slate-100">
                    ...
                    <strong className="text-slate-800">{prescription.vitals.bp}</strong>
                  </div>
                  <div className="bg-slate-50 p-1.5 rounded-lg border border-slate-100">
                    <span className="text-slate-400 block text-[9px]">Pulse</span>
                    <strong className="text-slate-800">{prescription.vitals.pulse}</strong>
                  </div>
                  <div className="bg-slate-50 p-1.5 rounded-lg border border-slate-100">
                    <span className="text-slate-400 block text-[9px]">Temp</span>
                    <strong className="text-slate-800">{prescription.vitals.temp}</strong>
                  </div>
                  <div className="bg-slate-50 p-1.5 rounded-lg border border-slate-100">
                    <span className="text-slate-400 block text-[9px]">Weight</span>
                    <strong className="text-slate-800">{prescription.vitals.weight}</strong>
                  </div>
                </div>
              </div>
            ...
            <div className="md:col-span-8 pl-0 md:pl-2 space-y-6">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-slate-400 font-semibold">(ঔষধ সেবন নির্দেশিকা)</span>
              </div>
              ...
              <div className="space-y-3.5">
                {prescription.medicines.map((m, idx) => (
                  <div
                    key={m.id}
                    className="p-3 rounded-2xl bg-slate-50 border border-slate-200/90 space-y-1"
                  >
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <h4 className="font-bold text-sm text-slate-900">
                          {m.brandName} <span className="text-xs text-slate-500 font-normal">({m.strength})</span>
                        </h4>
                      </div>
                    ...
                    <div className="text-xs text-slate-600 pl-7 flex flex-wrap items-center gap-x-4 gap-y-1">
                      <span>🍽️ {m.mealTiming}</span>
                      <span>⏱️ {m.durationBn}</span>
                      {m.specialInstruction && (
                        <span className="text-red-600 font-semibold">⚠️ {m.specialInstruction}</span>
                      )}
                    </div>
                  </div>
                ))}
              ...
              {/* Advice */}
              <div className="pt-2">
                <h4 className="font-bold text-xs text-slate-900 mb-2">পরামর্শ ও খাদ্য নির্দেশিকা:</h4>
