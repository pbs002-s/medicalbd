        ...
        <div className="p-4 bg-slate-50 dark:bg-slate-800/60 border-b border-slate-200 dark:border-slate-800 shrink-0">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs font-bangla focus:outline-hidden focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 transition-all"
            />
          </div>
        </div>

        {/* 2-Column Explorer */}
        <div className="grid grid-cols-1 md:grid-cols-12 divide-y md:divide-y-0 md:divide-x divide-slate-100 dark:divide-slate-800 overflow-y-auto flex-1 font-bangla">
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between">
                    <div>
                      <h4 className="font-bold text-sm text-slate-900 dark:text-slate-50 leading-tight">{med.brandName}</h4>
                      <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{med.genericName}</p>
                    </div>
          ...
          <div className="md:col-span-7 p-6 space-y-5 overflow-y-auto">
            ...
            <div className="p-5 rounded-3xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200/80 space-y-3">
              <div className="flex items-start justify-between">
                <div>
                ...
                <div className="text-right">
                  <span className="text-[10px] text-slate-400 dark:text-slate-500 block">সরকারি MRP মূল্য</span>
                  <span className="text-2xl font-black text-emerald-700 font-mono">
                    ৳ {toBn(selectedMedicine.mrpPrice.toFixed(2))}
                  </span>
                  <span className="text-[10px] text-slate-400 dark:text-slate-500 block">প্রতি পিস</span>
                </div>
              </div>

              <div className="grid grid-cols-2 gap-2 text-xs pt-2 border-t border-slate-200/60">
                <div>
                  <span className="text-slate-400 dark:text-slate-500 text-[10px] block">প্রস্তুতকারক প্রতিষ্ঠান:</span>
                  <strong className="text-slate-800 dark:text-slate-100">{selectedMedicine.company}</strong>
                </div>
                <div>
                  <span className="text-slate-400 dark:text-slate-500 text-[10px] block">গর্ভকালীন নিরাপত্তা:</span>
                  <strong className="text-blue-700">{selectedMedicine.pregnancyCategory}</strong>
                </div>
              </div>

              <div className="text-xs">
                <span className="text-slate-400 dark:text-slate-500 text-[10px] block">ব্যবহার / ইনডিকেশন:</span>
                <p className="text-slate-700 dark:text-slate-300">{selectedMedicine.indication}</p>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                {selectedMedicine.alternativeBrands.map((alt, i) => (
                  <div
                    key={i}
                    className="p-3 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-center justify-between hover:border-emerald-300 transition-colors"
                  >
                    <div>
                      <h5 className="font-bold text-xs text-slate-900 dark:text-slate-50">{alt.brandName}</h5>
                      <p className="text-[10px] text-slate-500 dark:text-slate-400">{alt.company}</p>
                    </div>
                    ...
                    <div className="text-right">
                      ...
                      <span className="text-[9px] text-slate-400 dark:text-slate-500 block">প্রতি পিস</span>
                    </div>
                  </div>
                ))}
              </div>
