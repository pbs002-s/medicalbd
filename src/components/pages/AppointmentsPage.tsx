    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6 font-bangla">
        ...
        <div className="space-y-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-base font-bold text-slate-900">আসন্ন চেম্বার অ্যাপয়েন্টমেন্টসমূহ</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {mockAppointments.map((apt, index) => (
                <ScrollReveal key={apt.id} animation="fade-up" delay={index * 120}>
                  <div className="bg-white rounded-3xl border border-slate-100/90 hover:border-blue-200 p-5 shadow-xs hover:shadow-md transition-all group">
                    ...
                    <div className="flex items-start justify-between gap-3 border-b border-slate-100 pb-3.5 mb-3.5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500">{apt.doctorSpecialtyBn}</p>
                        </div>
                      </div>
                      ...
                      <div className="text-right shrink-0">
                        ...
                        <div className="mt-1 text-xs text-slate-400 font-mono">
                          টোকেন <strong className="text-slate-800 text-sm font-black">#{toBn(apt.tokenNumber)}</strong>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="p-3 bg-slate-50 rounded-2xl border border-slate-100 space-y-1.5 text-xs text-slate-600">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-semibold text-slate-800">{apt.doctorHospitalBn}</span>
                      </div>
                    ...
                    <div className="flex items-center justify-between gap-2 mt-4 pt-2 border-t border-slate-50">
                      ...
                      <button
                        ...
                        className="p-2 rounded-xl border border-slate-200 hover:bg-slate-50 text-slate-600 text-xs font-semibold transition-colors btn-press"
      ...
      {activeTab === 'book_new' && (
        <ScrollReveal animation="zoom-in" duration={350}>
          <div className="bg-white rounded-3xl border border-slate-100 shadow-xs p-6 sm:p-8 max-w-3xl mx-auto">
            <div className="flex items-center gap-3 border-b border-slate-100 pb-4 mb-6">
              <div className="w-10 h-10 rounded-2xl bg-blue-50 text-blue-600 flex items-center justify-center">
                <Calendar className="w-5 h-5" />
              </div>
              <div>
                <h2 className="text-lg font-bold text-slate-900">নতুন ডাক্তার অ্যাপয়েন্টমেন্ট বুকিং</h2>
            ...
            <form onSubmit={handleBookingSubmit} className="space-y-5 text-xs">
              ...
              <div>
                <label className="block font-bold text-slate-700 mb-2">বিভাগ বা বিশেষত্ব নির্বাচন করুন:</label>
              ...
              {/* Doctor Selector */}
              <div>
                <label className="block font-bold text-slate-700 mb-1.5">চিকিৎসক নির্বাচন করুন:</label>
                <select
                  value={selectedDoctor}
                  onChange={(e) => setSelectedDoctor(e.target.value)}
                  className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-xs font-semibold text-slate-800 focus:outline-hidden focus:border-blue-500 transition-colors"
                >
                  {doctorsList.map((d) => (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block font-bold text-slate-700 mb-1.5">তারিখ নির্বাচন:</label>
                  <input
                    type="date"
                    required
                    value={selectedDate}
                    onChange={(e) => setSelectedDate(e.target.value)}
                    className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-xs font-mono text-slate-800 focus:outline-hidden focus:border-blue-500"
                  />
                </div>

                <div>
                  <label className="block font-bold text-slate-700 mb-1.5">চেম্বার সময় স্লট:</label>
              ...
              {/* Notes */}
              <div>
                <label className="block font-bold text-slate-700 mb-1">রোগীর শারীরিক সমস্যা (সংক্ষেপে):</label>
                <textarea
                  rows={2}
                  value={patientNotes}
                  onChange={(e) => setPatientNotes(e.target.value)}
                  placeholder="যেমন: ৩ দিন ধরে জ্বর ও কাশি..."
                  className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 text-xs focus:outline-hidden focus:border-blue-500"
                />
              </div>
              ...
              <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 flex items-center justify-between">
                <div>
                  <span className="text-slate-500 block text-[11px]">অনুমোদিত কনসালটেশন ফি:</span>
          ...
          <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-2xs flex flex-col sm:flex-row items-center justify-between gap-3">
            <div className="relative flex-1 w-full">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 rounded-xl bg-slate-50 border border-slate-200 text-xs focus:outline-hidden focus:border-blue-500 font-bangla"
              />
            </div>
            ...
            {doctorsList
              ...
              .map((doc, i) => (
                <ScrollReveal key={doc.id} animation="fade-up" delay={i * 100}>
                  <div className="bg-white rounded-3xl border border-slate-100 p-5 shadow-xs hover:shadow-md hover:border-blue-200 transition-all flex flex-col justify-between">
