    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6 font-bangla">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-6 space-y-5">
          ...
          <div className="bg-white rounded-3xl border border-slate-100 p-5 shadow-xs space-y-4 text-xs">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                <label className="block text-slate-500 mb-1">রোগীর নাম:</label>
                <input
                  type="text"
                  value={patientName}
              ...
              <div>
                <label className="block text-slate-500 mb-1">বয়স:</label>
                <input
                  type="text"
                  value={patientAge}
              ...
              <div>
                <label className="block text-slate-500 mb-1">লিঙ্গ:</label>
                <select
                  value={patientGender}
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                <label className="block text-slate-500 mb-1">রক্তচাপ (BP):</label>
                <input
                  type="text"
                  value={bp}
              ...
              <div>
                <label className="block text-slate-500 mb-1">পালস (Pulse):</label>
                <input
                  type="text"
                  value={pulse}
              ...
              <div>
                <label className="block text-slate-500 mb-1">ওজন (kg):</label>
                <input
                  type="text"
                  value={weight}
            ...
            <div>
              <label className="block text-slate-500 mb-1">রোগ নির্ণয় (Provisional Diagnosis):</label>
              <input
                type="text"
                value={diagnosis}
                onChange={(e) => setDiagnosis(e.target.value)}
                className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 font-semibold text-emerald-800"
              />
            </div>
          </div>

          {/* Quick Drug Search & Add */}
          <div className="bg-white rounded-3xl border border-slate-100 p-5 shadow-xs space-y-3 text-xs">
            <h3 className="font-bold text-slate-900 text-sm flex items-center gap-1.5">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2.5 rounded-xl bg-slate-50 border border-slate-200 font-bangla"
              />
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {filteredSearchResults.map((m) => (
                <button
                  ...
                  className="p-2.5 rounded-xl border border-slate-200 hover:border-emerald-500 hover:bg-emerald-50/50 text-left transition-all flex items-center justify-between"
                >
                  <div>
                    <span className="font-bold text-slate-900 block">{m.brandName}</span>
                    <span className="text-[10px] text-slate-500">{m.genericName} ({m.strength})</span>
                  </div>
        ...
        <div className="lg:col-span-6">
          <div className="bg-white rounded-3xl border border-slate-200 shadow-md p-6 space-y-5 text-xs">
            {/* Header */}
            <div className="border-b-2 border-emerald-600 pb-3 flex justify-between items-start">
              <div>
                <h3 className="font-black text-slate-900 text-base">{currentUser?.nameBn || 'ডা. তানভীর হাসান'}</h3>
                <p className="text-[11px] text-emerald-700">MBBS (DMC), FCPS (Medicine)</p>
                <p className="text-[10px] text-slate-400">BMDC Reg: A-54982</p>
              </div>
            ...
            <div className="space-y-3">
              ...
              {medsList.map((med, index) => (
                <div key={index} className="p-3 bg-slate-50 rounded-xl border border-slate-200 flex items-start justify-between gap-2">
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-bold text-slate-900">{index + 1}. {med.brandName}</span>
                      <span className="text-[10px] text-slate-500 font-mono">({med.strength})</span>
                    </div>
