    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6 font-bangla">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <ScrollReveal animation="fade-up" delay={50}>
          <div className="bg-white p-4 rounded-3xl border border-slate-100 shadow-2xs flex items-center gap-3.5">
            <div className="w-11 h-11 rounded-2xl bg-red-50 text-red-600 flex items-center justify-center shrink-0">
              <HeartPulse className="w-6 h-6 animate-pulse" />
            </div>
            <div>
              <span className="text-[11px] text-slate-500 font-medium block">মোট ফাঁকা ICU বেড</span>
              <span className="text-2xl font-black text-slate-900 leading-tight block">{toBn(totalIcuAvailable)} টি</span>
        ...
        <ScrollReveal animation="fade-up" delay={100}>
          <div className="bg-white p-4 rounded-3xl border border-slate-100 shadow-2xs flex items-center gap-3.5">
            ...
            <div>
              <span className="text-[11px] text-slate-500 font-medium block">মোট ফাঁকা CCU / HDU বেড</span>
              <span className="text-2xl font-black text-slate-900 leading-tight block">{toBn(totalCcuAvailable)} টি</span>
        ...
        <ScrollReveal animation="fade-up" delay={150}>
          <div className="bg-white p-4 rounded-3xl border border-slate-100 shadow-2xs flex items-center gap-3.5">
            ...
            <div>
              <span className="text-[11px] text-slate-500 font-medium block">সাধারণ ওয়ার্ড বেড ফাঁকা</span>
              <span className="text-2xl font-black text-slate-900 leading-tight block">{toBn(totalGeneralAvailable)} টি</span>
      ...
      {/* Filter Toolbar */}
      <div className="bg-white rounded-3xl border border-slate-100 p-4 shadow-xs flex flex-col md:flex-row items-center justify-between gap-3">
        {/* Search */}
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 rounded-xl bg-slate-50 border border-slate-200 text-xs focus:outline-hidden focus:border-cyan-500 font-bangla"
          />
        </div>
            ...
            {['all', 'Dhaka', 'Chittagong'].map((d) => (
              <button
                ...
                className={`px-3 py-1 rounded-lg text-xs font-semibold transition-all ${
                  selectedDistrict === d ? 'bg-cyan-600 text-white font-bold' : 'text-slate-600 hover:text-slate-900'
                }`}
              >
          ...
          <div className="flex gap-1 bg-slate-100 p-1 rounded-xl">
            {[
              ...
              <button
                ...
                className={`px-3 py-1 rounded-lg text-xs font-semibold transition-all ${
                  bedTypeFilter === bt.id ? 'bg-cyan-700 text-white font-bold' : 'text-slate-600 hover:text-slate-900'
                }`}
              >
                {bt.label}
              </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filteredHospitals.map((h, index) => (
          <ScrollReveal key={h.id} animation="fade-up" delay={index * 80}>
            <div className="bg-white rounded-3xl border border-slate-100 p-5 shadow-xs hover:shadow-md hover:border-cyan-200 transition-all space-y-4">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h3 className="font-bold text-slate-900 text-base">{h.hospitalNameBn}</h3>
                  <p className="text-xs text-slate-500 font-sans mt-0.5">{h.hospitalName}</p>
              ...
              <div className="grid grid-cols-4 gap-2 text-center text-xs">
                <div className="p-2.5 rounded-2xl bg-red-50/70 border border-red-100">
                  ...
                  <span className="text-[9px] text-slate-400">মোট {toBn(h.icuBeds.total)}</span>
                </div>
                ...
                <div className="p-2.5 rounded-2xl bg-cyan-50/70 border border-cyan-100">
                  ...
                  <span className="text-[9px] text-slate-400">মোট {toBn(h.ccuBeds.total)}</span>
                </div>
                ...
                <div className="p-2.5 rounded-2xl bg-emerald-50/70 border border-emerald-100">
                  ...
                  <span className="text-[9px] text-slate-400">মোট {toBn(h.generalBeds.total)}</span>
                </div>
                ...
                <div className="p-2.5 rounded-2xl bg-purple-50/70 border border-purple-100">
                  ...
                  <span className="text-[9px] text-slate-400">মোট {toBn(h.nicuBeds.total)}</span>
                </div>
              </div>
