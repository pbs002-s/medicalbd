    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6 font-bangla">
        ...
        <div className="p-5 rounded-3xl bg-gradient-to-r from-red-600 via-rose-600 to-red-700 text-white shadow-md flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <a
              ...
              className="px-4 py-2.5 bg-white text-red-700 hover:bg-red-50 rounded-2xl text-xs font-bold flex items-center gap-1.5 shadow-sm transition-all btn-press"
            >
      ...
      <div className="bg-white rounded-3xl border border-slate-100 p-4 shadow-xs space-y-3">
        <div className="flex items-center justify-between">
          <span className="text-xs font-bold text-slate-800">রক্তের গ্রুপ অনুযায়ী খুঁজুন:</span>
            ...
            return (
              <button
                ...
                className={`px-4 py-2 rounded-2xl text-xs font-mono font-black transition-all ${
                  isSelected
                    ? 'bg-red-600 text-white shadow-xs'
                    : 'bg-slate-50 text-slate-700 hover:bg-red-50 hover:text-red-700 border border-slate-200'
                }`}
              >
                {bg} ({toBn(count)})
              </button>
      ...
      {/* Search & Location Bar */}
      <div className="bg-white rounded-3xl border border-slate-100 p-4 shadow-xs flex flex-col sm:flex-row items-center justify-between gap-3">
        <div className="relative w-full sm:w-80">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 rounded-xl bg-slate-50 border border-slate-200 text-xs focus:outline-hidden focus:border-red-500 font-bangla"
          />
        </div>

        <div className="flex items-center gap-2 w-full sm:w-auto">
          <span className="text-xs text-slate-500 font-bold shrink-0">জেলা:</span>
          <select
            value={selectedDistrict}
            onChange={(e) => setSelectedDistrict(e.target.value)}
            className="px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bangla focus:outline-hidden"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredDonors.map((donor, index) => (
          <ScrollReveal key={donor.id} animation="fade-up" delay={index * 60}>
            <div className="bg-white rounded-3xl border border-slate-100 p-5 shadow-xs hover:shadow-md hover:border-red-200 transition-all space-y-3.5">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-slate-900 text-sm leading-tight">{donor.name}</h3>
              ...
              <div className="grid grid-cols-2 gap-2 text-xs bg-slate-50 p-2.5 rounded-2xl border border-slate-100">
                <div>
                  <span className="text-slate-400 text-[10px] block">সর্বশেষ রক্তদান</span>
                  <span className="font-mono font-semibold text-slate-700">{donor.lastDonationDate}</span>
                </div>
                <div>
      ...
      {isRegisterDonorOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in">
          <div className="bg-white rounded-3xl max-w-md w-full p-6 space-y-4 shadow-2xl border border-slate-100 animate-slide-up">
            <h3 className="text-base font-bold text-slate-900">রক্তদাতা হিসেবে নিবন্ধন করুন</h3>
            <div className="space-y-3 text-xs">
              <div>
                <label className="text-slate-700 font-bold block mb-1">আপনার পূর্ণ নাম</label>
                <input
                  type="text"
                  placeholder="যেমন: সালমান আহমেদ"
                  className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 font-bangla"
                />
              </div>
              <div>
                <label className="text-slate-700 font-bold block mb-1">রক্তের গ্রুপ</label>
                <select className="w-full p-2.5 rounded-xl bg-slate-50 border border-slate-200 font-bold">
                  {bloodGroups.map((bg) => (
              ...
              <div>
                <label className="text-slate-700 font-bold block mb-1">মোবাইল নম্বর</label>
                <input
                  type="tel"
              ...
              <div>
                <label className="text-slate-700 font-bold block mb-1">জেলা ও থানা</label>
                <input
                  type="text"
