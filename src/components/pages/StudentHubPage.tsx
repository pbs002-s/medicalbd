    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6 font-bangla">
          ...
          <div className="flex items-center gap-1.5 bg-slate-100/90 p-1 rounded-2xl overflow-x-auto text-xs font-bold">
            <button
              ...
              className={`px-3 py-1.5 rounded-xl transition-all whitespace-nowrap flex items-center gap-1.5 ${
                activeTab === 'logbook' ? 'bg-purple-600 text-white shadow-xs' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:text-slate-50'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl transition-all whitespace-nowrap flex items-center gap-1.5 ${
                activeTab === 'osce' ? 'bg-blue-600 text-white shadow-xs' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:text-slate-50'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl transition-all whitespace-nowrap flex items-center gap-1.5 ${
                activeTab === 'dose' ? 'bg-amber-600 text-white shadow-xs' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:text-slate-50'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl transition-all whitespace-nowrap flex items-center gap-1.5 ${
                activeTab === 'quiz' ? 'bg-emerald-600 text-white shadow-xs' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:text-slate-50'
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl transition-all whitespace-nowrap flex items-center gap-1.5 ${
                activeTab === 'forum' ? 'bg-indigo-600 text-white shadow-xs' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:text-slate-50'
              }`}
            >
      ...
      {activeTab === 'logbook' && (
        <div className="space-y-6">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 shadow-2xs">
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold text-slate-700 dark:text-slate-300">বিভাগ ফিল্টার:</span>
                ...
                {['all', 'Medicine', 'Surgery', 'Gynae', 'Pediatrics'].map((w) => (
                  <button
                    ...
                    className={`px-3 py-1 rounded-xl text-xs font-semibold transition-colors ${
                      selectedDept === w ? 'bg-purple-600 text-white font-bold' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400'
                    }`}
                  >
                    {w === 'all' ? 'সকল ওয়ার্ড' : w}
                  </button>
            ...
            {mockClinicalCases
              ...
              .map((c, i) => (
                <ScrollReveal key={c.id} animation="fade-up" delay={i * 80}>
                  <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 p-5 shadow-xs hover:shadow-md hover:border-purple-200 transition-all space-y-3">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        ...
                        <h3 className="font-bold text-slate-900 dark:text-slate-50 text-sm mt-1">{c.title}</h3>
                        <p className="text-[11px] text-slate-500 dark:text-slate-400">{c.patientAgeGender}</p>
                      </div>
      ...
      {activeTab === 'osce' && (
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-md p-6 sm:p-8 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 border-b border-slate-100 dark:border-slate-800 pb-5">
                ...
                <button
                  ...
                  className={`px-3.5 py-2 rounded-xl text-xs font-bold transition-all ${
                    selectedStation.id === st.id
                      ...
                      : 'bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'
                  }`}
                >
                  {st.title}
                </button>
            ...
            {/* Timer & Score Box */}
            <div className="flex items-center gap-3">
              <div className="flex items-center gap-2 bg-slate-50 dark:bg-slate-800/60 px-3.5 py-2 rounded-2xl border border-slate-200 dark:border-slate-800 font-mono font-bold text-xs">
                <Timer className="w-4 h-4 text-blue-600" />
                <span className={`text-base ${timeLeftSeconds < 60 ? 'text-red-600 animate-pulse font-black' : 'text-slate-800 dark:text-slate-100'}`}>
                ...
                <button onClick={resetTimer} className="p-1 rounded-lg bg-slate-200 text-slate-600 dark:text-slate-400 hover:bg-slate-300">
          ...
          <div className="space-y-3 text-xs">
            <h4 className="font-bold text-slate-900 dark:text-slate-50 text-sm">পরীক্ষকের স্টেপ-বাই-স্টেপ মার্কিং চেকলিস্ট:</h4>
                ...
                <div key={i} className="p-3.5 rounded-2xl bg-purple-50/50 border border-purple-100 space-y-2">
                  <div className="flex items-start justify-between gap-2">
                    ...
                    <button
                      ...
                      className="px-2.5 py-1 bg-white dark:bg-slate-900 hover:bg-purple-100 text-purple-700 rounded-lg text-[10px] font-bold border border-purple-200 shrink-0"
                    >
      ...
      {activeTab === 'dose' && (
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-md p-6 sm:p-8 max-w-3xl mx-auto space-y-6">
          <div className="flex items-center gap-3 border-b border-slate-100 dark:border-slate-800 pb-4">
            <div className="w-10 h-10 rounded-2xl bg-amber-50 text-amber-600 flex items-center justify-center">
              <Calculator className="w-5 h-5" />
            </div>
            <div>
              <h2 className="text-lg font-bold text-slate-900 dark:text-slate-50">পেডিয়াট্রিক mg/kg ওজনভিত্তিক ডোজ ক্যালকুলেটর</h2>
              <p className="text-xs text-slate-500 dark:text-slate-400">শিশুর সঠিক ওজন অনুযায়ী সিরাপ ও ড্রপসের নিখুঁত পরিমাণ হিসেব</p>
            </div>
          </div>
          ...
          <div className="p-6 rounded-3xl bg-slate-900 text-white space-y-4 shadow-md">
            <div className="flex items-start justify-between border-b border-slate-800 pb-3">
              <div>
                ...
                <p className="text-xs text-slate-400 dark:text-slate-500 mt-0.5">{doseResult.standardRule}</p>
              </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-md p-6 sm:p-8 max-w-3xl mx-auto space-y-6">
          <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h3 className="text-base font-bold text-slate-900 dark:text-slate-50">FCPS Part-1 ও রেসিডেন্সি মক টেস্ট</h3>
                <p className="text-xs text-slate-500 dark:text-slate-400">প্রশ্ন {toBn(currentQIndex + 1)} / {toBn(mockPostGradQuestions.length)}</p>
              </div>
            </div>
          ...
          <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-800">
            <h4 className="text-sm sm:text-base font-bold text-slate-900 dark:text-slate-50 leading-relaxed">
              {currentQ.question}
            </h4>
          </div>
          ...
          <div className="flex justify-between pt-4 border-t border-slate-100 dark:border-slate-800">
            <button
              ...
              className="px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-800 text-xs font-bold text-slate-700 dark:text-slate-300 disabled:opacity-40"
            >
              পূর্ববর্তী প্রশ্ন
            </button>
            <button
      ...
      {activeTab === 'forum' && (
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 p-5 shadow-xs space-y-3">
            <h3 className="font-bold text-slate-900 dark:text-slate-50 text-sm">নতুন ক্লিনিক্যাল কেস বা ECG আলোচনা পোস্ট করুন</h3>
            <textarea
              rows={2}
              value={newPostContent}
              onChange={(e) => setNewPostContent(e.target.value)}
              placeholder="কেসের উপসর্গ, ইসিজি বা হিস্ট্রি শেয়ার করুন এবং সিনিয়রদের মতামত নিন..."
              className="w-full p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-800 text-xs focus:outline-hidden focus:border-indigo-500 font-bangla"
            />
          ...
          <div className="space-y-4">
            {forumPostsList.map((post: ForumPost, i: number) => (
              <ScrollReveal key={post.id} animation="fade-up" delay={i * 80}>
                <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800 p-5 sm:p-6 shadow-xs hover:shadow-md transition-all space-y-3">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <div>
                        <h4 className="font-bold text-slate-900 dark:text-slate-50 text-xs sm:text-sm">{post.authorNameBn}</h4>
                        <p className="text-[11px] text-slate-400 dark:text-slate-500">{post.authorTitleBn} • {post.createdAtBn}</p>
                      </div>
                    </div>
                  ...
                  <h3 className="font-bold text-slate-900 dark:text-slate-50 text-sm">{post.title}</h3>
                  ...
                  <div className="pt-2 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between text-xs">
                    <div className="flex items-center gap-3">
                      <button
                        ...
                        className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border text-xs font-bold transition-all ${
                          likedPosts[post.id]
                            ...
                            : 'bg-slate-50 dark:bg-slate-800/60 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 border-slate-200 dark:border-slate-800'
                        }`}
                      >
                      ...
                      <button className="flex items-center gap-1.5 text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:text-slate-100 font-semibold">
                        <MessageCircle className="w-3.5 h-3.5" />
                        <span>{toBn(post.commentsCount)}টি মন্তব্য</span>
                      </button>
                    </div>

                    <button className="p-1.5 text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:text-slate-300">
