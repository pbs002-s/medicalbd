          ...
          <div className="bg-white rounded-3xl border border-slate-100 p-4 shadow-xs space-y-3">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 rounded-xl bg-slate-50 border border-slate-200 text-xs focus:outline-hidden focus:border-blue-500 font-bangla"
              />
            </div>
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <span className="text-[10px] font-mono font-bold text-blue-600 bg-white px-2 py-0.5 rounded-md border border-blue-100 inline-block mb-1">
                          {rx.prescriptionNumber}
                        </span>
                        <h4 className="font-bold text-slate-900 text-xs leading-tight">{rx.doctorNameBn}</h4>
                        <p className="text-[11px] text-slate-500 mt-0.5">{rx.doctorHospital}</p>
                      </div>
                      <span className="text-[10px] text-slate-400 font-bangla shrink-0">{rx.dateBn}</span>
                    </div>
        ...
        <div className="lg:col-span-8">
          <ScrollReveal animation="fade-up" duration={450}>
            <div
              ...
              className="bg-white rounded-3xl border border-slate-200/90 shadow-md p-6 sm:p-10 space-y-6 relative overflow-hidden"
            >
              ...
              <div className="border-b-2 border-blue-600 pb-5 flex flex-col sm:flex-row justify-between items-start gap-4">
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">{currentRx.doctorHospital}</p>
                ...
                <div className="text-left sm:text-right text-xs text-slate-500 space-y-0.5">
                  <p className="font-bold text-slate-800">ল্যাবএইড স্পেশালাইজড চেম্বার</p>
                  <p className="text-[11px] text-slate-500 max-w-xs">বাড়ি #০১, রোড #০৪, ধানমন্ডি, ঢাকা</p>
                  <p className="font-mono text-blue-600 font-bold">হটলাইন: 09678123456</p>
                </div>
              </div>

              {/* Patient Banner Bar */}
              <div className="p-3.5 bg-slate-50 rounded-2xl border border-slate-200 grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
                <div>
                  <span className="text-slate-400 block text-[10px]">রোগীর নাম</span>
                  <strong className="text-slate-900 font-bold">{currentRx.patientNameBn}</strong>
                </div>
                <div>
                  <span className="text-slate-400 block text-[10px]">বয়স ও লিঙ্গ</span>
                  <span className="text-slate-800 font-semibold">{toBn(currentRx.patientAge)} বছর / {currentRx.patientGender}</span>
                </div>
                <div>
                  <span className="text-slate-400 block text-[10px]">তারিখ</span>
                  <span className="text-slate-800 font-mono font-semibold">{currentRx.dateBn}</span>
                </div>
                <div>
                  <span className="text-slate-400 block text-[10px]">প্রেসক্রিপশন আইডি</span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-6 pt-2">
                ...
                <div className="md:col-span-4 space-y-5 border-r md:border-slate-200/80 pr-0 md:pr-4">
                  ...
                  {/* Complaints */}
                  <div>
                    <h4 className="text-xs font-bold text-slate-900 uppercase tracking-wider mb-1.5">রোগীর প্রধান উপসর্গ (Complaints)</h4>
                  ...
                  {currentRx.investigations && currentRx.investigations.length > 0 && (
                    <div>
                      <h4 className="text-xs font-bold text-slate-900 uppercase tracking-wider mb-1.5">প্রস্তাবিত পরীক্ষা (Lab Tests)</h4>
                      <ul className="space-y-1 text-xs text-slate-700">
                        {currentRx.investigations.map((test, i) => (
                          <li key={i} className="p-2 bg-slate-50 rounded-lg border border-slate-100 font-mono text-[11px]">
                            {i + 1}. {test}
                          </li>
                        ))}
                      </ul>
                ...
                <div className="md:col-span-8 space-y-4">
                  <div className="flex items-center gap-2 pb-2 border-b border-slate-200">
                    ...
                    <span className="text-xs text-slate-400 font-sans">(নির্ধারিত ওষুধ ও সেবনবিধি)</span>
                  </div>
                  ...
                  <div className="space-y-3">
                    {currentRx.medicines.map((med, index) => (
                      <div
                        key={med.id}
                        className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200/80 hover:bg-blue-50/30 transition-colors space-y-1.5"
                      >
                        <div className="flex items-start justify-between gap-2">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              <h5 className="font-bold text-slate-900 text-sm">
                                {med.brandName} <span className="text-xs font-normal text-slate-500">({med.genericName})</span>
                              </h5>
                          ...
                          <div className="text-right">
                            <span className="px-2.5 py-1 bg-white border border-slate-200 rounded-lg font-mono font-black text-xs text-slate-800 shadow-2xs">
                              {med.frequencyBn}
                            </span>
                          </div>
                        </div>

                        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-600 pt-1 pl-7">
                          <span>⏱️ নিয়ম: <strong className="text-slate-800">{med.mealTiming}</strong></span>
                          <span>📅 মেয়াদ: <strong className="text-slate-800">{med.durationBn}</strong></span>
              ...
              <div className="border-t-2 border-slate-100 pt-5 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-slate-50 border border-slate-200 rounded-xl">
                    <QrCode className="w-12 h-12 text-slate-800" />
                  </div>
                  <div className="text-[11px] text-slate-400">
                    <p className="font-semibold text-slate-700">ডিজিটাল ভেরিফায়েড ই-প্রেসক্রিপশন</p>
                ...
                <div className="text-center sm:text-right">
                  ...
                  <span className="text-xs font-bold text-slate-800 block">{currentRx.doctorNameBn}</span>
                  <span className="text-[10px] text-slate-400">ডিজিটাল স্বাক্ষর ও সিল</span>
                </div>
              </div>
            </div>
          </ScrollReveal>
