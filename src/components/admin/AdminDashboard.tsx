    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto space-y-6 font-bangla">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-2xs">
          <span className="text-[11px] text-slate-500 font-medium block">আজকের মোট অ্যাপয়েন্টমেন্ট</span>
          <span className="text-2xl font-black text-slate-900 my-0.5 block">{toBn(totalTokens)} টি</span>
          <span className="text-[10px] text-emerald-600 font-bold">সকল চেম্বার মিলিয়ে</span>
        </div>

        <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-2xs">
          <span className="text-[11px] text-slate-500 font-medium block">অন-ডিউটি কনসালট্যান্ট</span>
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-2xs">
          <span className="text-[11px] text-slate-500 font-medium block">মোট খালি ICU বেড</span>
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-2xs">
          <span className="text-[11px] text-slate-500 font-medium block">গড় অপেক্ষার সময়</span>
          <span className="text-2xl font-black text-emerald-600 my-0.5 block">{toBn(18)} মিনিট</span>
          <span className="text-[10px] text-slate-400">সিরিয়াল অপটিমাইজড</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="space-y-3">
            {beds.map((b) => (
              <div key={b.id} className="p-4 rounded-2xl bg-slate-50 border border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                  <h4 className="font-bold text-xs text-slate-900">{b.hospitalNameBn}</h4>
                  <p className="text-[11px] text-slate-500">{b.address}</p>
                </div>

                <div className="flex items-center gap-3 text-xs">
                  <div className="text-center">
                    <span className="text-[10px] text-slate-400 block">সাধারণ বেড</span>
                    <strong className="text-slate-800">{toBn(b.generalBeds.available)} খালি</strong>
                  </div>
                  <div className="text-center border-l pl-3 border-slate-200">
        ...
        <div className="lg:col-span-4 bg-white p-6 rounded-3xl border border-slate-100 shadow-xs space-y-3">
          <h3 className="font-bold text-slate-900 text-sm">ডাক্তার রোস্টার ও BMDC যাচাইকরণ</h3>
          ...
          <div className="p-3 bg-emerald-50 rounded-2xl border border-emerald-200 space-y-1 text-xs">
            ...
            <p className="text-slate-600 text-[11px]">মেডিসিন বিশেষজ্ঞ • রুম ৩০৪</p>
          </div>
          ...
          <div className="p-3 bg-blue-50 rounded-2xl border border-blue-200 space-y-1 text-xs">
            ...
            <p className="text-slate-600 text-[11px]">কার্ডিওলজি স্পেশালিস্ট • রুম ২০২</p>
          </div>
        </div>
      </div>
    </div>
