        ...
        <div className="p-6 sm:p-8 rounded-3xl bg-gradient-to-r from-emerald-600 via-teal-700 to-slate-900 text-white shadow-md flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            <button
              ...
              className="px-4 py-2.5 bg-white text-emerald-800 hover:bg-emerald-50 rounded-2xl text-xs font-bold flex items-center gap-2 shadow-sm transition-all btn-press"
            >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <ScrollReveal animation="fade-up" delay={50}>
          <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-2xs">
            <span className="text-[11px] text-slate-500 font-medium block">আজকের মোট রোগী</span>
            <span className="text-2xl font-black text-slate-900 my-0.5 block">{toBn(totalTokens)} জন</span>
        ...
        <ScrollReveal animation="fade-up" delay={100}>
          <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-2xs">
            <span className="text-[11px] text-slate-500 font-medium block">দেখা সম্পন্ন হয়েছে</span>
            <span className="text-2xl font-black text-blue-600 my-0.5 block">{toBn(currentSerial - 1)} জন</span>
            <span className="text-[10px] text-slate-400">মোট রোগীর {(Math.round(((currentSerial - 1) / totalTokens) * 100))}%</span>
          </div>
        </ScrollReveal>

        <ScrollReveal animation="fade-up" delay={150}>
          <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-2xs">
            <span className="text-[11px] text-slate-500 font-medium block">চলতি সিরিয়াল</span>
        ...
        <ScrollReveal animation="fade-up" delay={200}>
          <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-2xs">
            <span className="text-[11px] text-slate-500 font-medium block">১৪ দিনের ফ্রি রিপোর্ট রোগী</span>
                  ...
                  return (
                    <div
                      ...
                      className={`p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 transition-colors ${
                        isCurrent ? 'bg-emerald-50/70 font-semibold' : 'hover:bg-slate-50'
                      }`}
                    >
                      <div className="flex items-center gap-3">
                        <div className={`w-10 h-10 rounded-2xl flex items-center justify-center font-mono font-black text-sm ${
                          isCurrent ? 'bg-emerald-600 text-white shadow-xs' : 'bg-slate-100 text-slate-700'
                        }`}>
                          #{toBn(p.token)}
                        </div>
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs text-slate-400">({toBn(p.age)} বছর)</span>
                            {p.isReport && (
                          ...
                          <p className="text-xs text-slate-500 mt-0.5">{p.problem}</p>
                        </div>
                      </div>
        ...
        <div className="lg:col-span-4 space-y-4">
          <ScrollReveal animation="fade-up" delay={150}>
            <div className="bg-white p-5 rounded-3xl border border-slate-100 shadow-xs space-y-3">
              <h3 className="font-bold text-slate-900 text-sm">আমার চেম্বার শিডিউল</h3>

              <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200 space-y-1 text-xs">
                <span className="text-[10px] font-bold px-2 py-0.5 bg-emerald-100 text-emerald-800 rounded-md">
                  সকালের সেশন (সরকারি)
                </span>
                <h4 className="font-bold text-slate-900">ঢাকা মেডিকেল কলেজ হাসপাতাল</h4>
                <p className="text-[11px] text-slate-500">শনিবার - বৃহস্পতিবার (সকাল ৮:০০ - দুপুর ২:৩০)</p>
              </div>
              ...
              <div className="p-3.5 rounded-2xl bg-blue-50/60 border border-blue-200 space-y-1 text-xs">
                ...
                <h4 className="font-bold text-slate-900">ল্যাবএইড ডায়াগনস্টিক, ধানমন্ডি</h4>
                <p className="text-[11px] text-slate-500">সন্ধ্যা ৫:০০ - রাত ৯:০০ (ফি: ৳ ১২০০)</p>
